...
class ReleaseController {
  async release ({ request, response, transform }) {
    const loanDetails = request.only(['loan_id'])

    // Process
    let loan = await transform.item(LoanRepository.release(loanDetails), 'LoanTransformer')
    ...
    const responseCode = Config.get('response.code.success.loan.release')
